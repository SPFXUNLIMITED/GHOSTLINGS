...
$current_quick_order_page = basename((string)($_SERVER['PHP_SELF'] ?? 'quick_order_list.php'));
$default_view = $current_quick_order_page === 'quick_order_list.php' ? 'all' : '';
$view = (string)($_GET['view'] ?? $default_view);
$show_all = $view === 'all';
  ...
  if (!hash_equals((string)$_SESSION['quick_order_csrf'], $csrf)) {
    ...
    if ($post_action === 'status') {
      ...
      if ($row_id <= 0) {
        ...
        $del->execute([$row_id]);
        $_SESSION['quick_order_csrf'] = bin2hex(random_bytes(24));
        header('Location: quick_order_list.php?view=all&deleted=1');
        exit;
      }
    }
    ...
    if (!$errors) {
      if ($edit_id !== null) {
        ...
        $_SESSION['quick_order_csrf'] = bin2hex(random_bytes(24));
        header('Location: quick_order_list.php?view=id&id=' . $edit_id . '&updated=1');
        exit;
      } else {
        // Insert new record
       ...
       $_SESSION['quick_order_csrf'] = bin2hex(random_bytes(24));
       header('Location: quick_order_list.php?view=id&id=' . $new_id . '&saved=1');
       exit;
       }
     }
    }
...
if ($show_detail) {
  ...
  if (!$detail_inquiry) {
    ...
    <div class="card">
      ...
      <div class="actions">
        <a class="btn" href="quick_order_list.php?view=all">Back to All Quick Orders</a>
        ...
        <tbody>
          ...
          <?php foreach ($inquiries as $inquiry): ?>
            ...
            <tr>
              ...
              <td style="white-space:nowrap;">
                <a class="btn" href="quick_order_list.php?view=id&id=<?= (int)$inquiry['id'] ?>">View</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
