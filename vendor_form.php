...
$fields = [
  ...
  'logo_path'     => '',
  'logo_thumb'    => '',
  'alibaba_profile_photo_path'  => '',
  'alibaba_profile_photo_thumb' => '',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!hash_equals((string)$_SESSION['vendor_form_csrf'], $csrf)) {
    ...
    foreach ($fields as $k => $_) {
      if (in_array($k, ['logo_path', 'logo_thumb', 'alibaba_profile_photo_path', 'alibaba_profile_photo_thumb'], true)) continue;
    ...
    [$new_logo_path, $new_logo_thumb] = $processImageUpload('logo_image', 'Logo', 'vendor_logo');
    [$new_alibaba_profile_photo_path, $new_alibaba_profile_photo_thumb] = $processImageUpload('alibaba_profile_photo_image', 'Alibaba profile photo', 'vendor_alibaba_profile_photo');
      ...
      if ($is_edit) {
        $pdo->prepare("
          UPDATE vendors SET
            ...
            alibaba_profile_photo_path = COALESCE(?, alibaba_profile_photo_path),
            alibaba_profile_photo_thumb = COALESCE(?, alibaba_profile_photo_thumb)
          WHERE id = ?
        ")->execute([
          ...
          $new_logo_path,
          $new_logo_thumb,
          $new_alibaba_profile_photo_path,
          $new_alibaba_profile_photo_thumb,
          $id,
        ]);
        ...
        $pdo->prepare("
          INSERT INTO vendors (company_name, contact_name, email, phone, port, website, alibaba_store, address, notes, rating, review, logo_path, logo_thumb, alibaba_profile_photo_path, alibaba_profile_photo_thumb)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ")->execute([
          $fields['company_name'], $fields['contact_name'], $fields['email'],
          $fields['phone'], $fields['port'], $fields['website'], $fields['alibaba_store'],
...
<div class="card">
  ...
  <form method="post" enctype="multipart/form-data" action="vendor_form.php<?= $is_edit ? '?id=' . $id : '' ?>" novalidate>
    ...
    <div class="form-grid">
      ...
      <div class="full">
        <label>Alibaba Profile Photo (JPG, PNG, GIF)</label>
        <input type="file" name="alibaba_profile_photo_image" id="alibaba_profile_photo_image" accept="image/jpeg,image/png,image/gif" />
        <div class="muted" style="margin-top:4px;">Optional. Upload the vendor's Alibaba profile photo.</div>
      </div>
      <div class="full">
      ...
      <div class="full">
        ...
        <?php
          $alibaba_profile_thumb_url = $fields['alibaba_profile_photo_thumb'] !== ''
            ? 'uploads/' . rawurlencode($fields['alibaba_profile_photo_thumb'])
            : 'uploads/' . rawurlencode($fields['alibaba_profile_photo_path']);
          $alibaba_profile_full_url  = $fields['alibaba_profile_photo_path'] !== ''
            ? 'uploads/' . rawurlencode($fields['alibaba_profile_photo_path'])
        ...
        <a href="<?= h($alibaba_profile_full_url) ?>" target="_blank" rel="noopener noreferrer" title="View Alibaba profile photo">
          <img src="<?= h($alibaba_profile_thumb_url) ?>"
               alt="Alibaba profile photo thumbnail"
               loading="lazy"
               decoding="async"
