...
$stmt = $pdo->prepare("SELECT id, name, owner_id FROM projects WHERE id = ? AND is_sop_category = 1");
...
render_header('SOP Pages');
?>
<div class="card">
  <div class="row" style="justify-content:space-between; align-items:center;">
    <div>
      <h1 style="margin:0;">SOP Pages</h1>
      <div class="muted">SOP Category: <strong><?= h($category['name']) ?></strong> (ID <?= (int)$category_id ?>)</div>
    </div>
    <div class="actions">
      <a class="btn" href="sops.php">Back to SOP</a>
      <a class="btn primary" href="sop_page_form.php?category_id=<?= (int)$category_id ?>">+ New SOP Page</a>
    </div>
  </div>
</div>
...
<div class="card">
  <table>
    ...
    <tbody>
      <?php if (!$documents): ?>
        <tr><td colspan="7" class="muted">No SOP pages yet.</td></tr>
      <?php endif; ?>
      ...
      <?php foreach ($documents as $d): ?>
        <tr>
          ...
          <td>
            <div class="actions">
              <a class="btn" href="sop_page_form.php?category_id=<?= (int)$category_id ?>&id=<?= (int)$d['id'] ?>">Edit</a>
              <a class="btn danger"
                 href="sop_page_delete.php?category_id=<?= (int)$category_id ?>&id=<?= (int)$d['id'] ?>"
                 onclick="return confirm('Delete this SOP page?');">
                Delete
              </a>
            </div>
          </td>
