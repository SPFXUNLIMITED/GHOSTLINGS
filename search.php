...
if ($search !== '') {
  ...
  $document_categories = $stmt->fetchAll(PDO::FETCH_ASSOC);

  if (is_admin()) {
    $stmt = $pdo->prepare("
      SELECT
        t.id, t.title, t.status, t.priority, t.created_at,
        p.id AS category_id, p.name AS category_name
      FROM tasks t
      JOIN projects p ON p.id = t.project_id
      WHERE p.is_doc_category = 1
        AND p.archived = 0
...
<div class="card">
  ...
  <p class="muted" style="margin:0;">Search projects, playbooks, documents, tasks, and files.</p>
</div>
...
<?php else: ?>
  ...
  <div class="card">
    <h2 style="margin-top:0;">Document Categories</h2>
    <?php if (!$document_categories): ?>
      <div class="muted">No matching document categories found.</div>
    <?php else: ?>
      <table class="table-auto">
        <thead>
          <tr>
          ...
          <?php foreach ($document_categories as $category): ?>
            <tr>
              <td>
                ...
                <span class="muted"><?= h($category['description'] ?? '') ?></span>
              </td>
              <td class="col-status">
                <span class="badge priority-<?= h($category['priority'] ?? 'medium') ?>">
                  <?= h(ucfirst($category['priority'] ?? 'medium')) ?>
                </span>
              </td>
              <td class="col-actions">
                <a class="btn" href="doc_tasks.php?category_id=<?= (int)$category['id'] ?>">Open</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
  ...
  <div class="card">
    ...
    <?php if (!$documents): ?>
      <div class="muted">No matching documents found.</div>
    <?php else: ?>
      <table class="table-auto">
        <thead>
          <tr>
        ...
        <tbody>
          <?php foreach ($documents as $document): ?>
            <tr>
              ...
              <td class="col-status"><?= h($document['category_name']) ?></td>
              <td class="col-status"><span class="badge <?= h($document['status']) ?>"><?= h($document['status']) ?></span></td>
              <td class="col-status">
                <span class="badge priority-<?= h($document['priority'] ?? 'medium') ?>">
                  <?= h(ucfirst($document['priority'] ?? 'medium')) ?>
                </span>
              </td>
              <td class="col-actions">
                <a class="btn" href="doc_form.php?category_id=<?= (int)$document['category_id'] ?>&id=<?= (int)$document['id'] ?>">Open</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
