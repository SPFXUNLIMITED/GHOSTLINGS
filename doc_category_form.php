...
if ($id) {
  $stmt = $pdo->prepare("SELECT id, name, description, priority, owner_id FROM projects WHERE id = ? AND is_doc_category = 1");
  $stmt->execute([$id]);
  $category = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    ...
    if ($id) {
      ...
      $stmt = $pdo->prepare("INSERT INTO projects (name, description, is_doc_category, playbook, priority, owner_id) VALUES (?, ?, 1, 0, ?, ?)");
