...
$category = ['name' => '', 'description' => '', 'playbook' => 0, 'is_doc_category' => 1, 'is_sop_category' => 0, 'priority' => 'medium', 'owner_id' => current_user_id(), 'app_category' => 'document'];

$all_users = is_admin() ? $pdo->query("SELECT id, username FROM users ORDER BY username")->fetchAll() : [];

if ($id) {
  $stmt = $pdo->prepare("SELECT id, name, description, playbook, is_doc_category, is_sop_category, priority, owner_id FROM projects WHERE id = ? AND is_doc_category = 1");
  $stmt->execute([$id]);
  $category = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $app_category = $_POST['app_category'] ?? ($category['app_category'] ?? 'document');
...
render_header($id ? 'Edit Category' : 'New Category');
?>
<div class="card">
  <div class="row" style="justify-content:space-between; align-items:center;">
    <h1 style="margin:0;"><?= $id ? 'Edit Category' : 'New Category' ?></h1>
  ...
  <form method="post">
    ...
    <div class="row" style="margin-top:12px;">
      ...
      <a class="btn" href="documents.php">Cancel</a>
    </div>
  </form>
</div>
<?php render_footer(); ?>
