...
if (is_admin()) {
  $cat_total = (int)$pdo->query("SELECT COUNT(*) FROM projects WHERE is_doc_category = 1 AND archived = 0")->fetchColumn();

  $stmt = $pdo->prepare("
    SELECT id, name, description, created_at, priority
    FROM projects
    WHERE is_doc_category = 1 AND archived = 0
    ORDER BY id DESC
  ...
  $doc_total = (int)$pdo->query("
    ...
    WHERE p.is_doc_category = 1 AND p.archived = 0
  ")->fetchColumn();
  ...
  $stmt = $pdo->prepare("
    ...
    WHERE pr.is_doc_category = 1 AND pr.archived = 0
...
<div class="card">
  <div class="row" style="justify-content:space-between; align-items:center;">
    ...
    <a class="btn primary" href="doc_category_form.php">+ New Category</a>
  </div>
  <p class="muted">Create categories, then manage documents inside each category.</p>
</div>

<div class="card">
  <div class="table-wrap">
    <table class="table-auto" data-table-key="doc-categories" data-default-sort-col="priority" data-default-sort-dir="desc">
      <thead>
        <tr>
          <th>
            <button type="button" class="linklike" data-sort-col="name" data-sort-type="text" aria-label="Sort categories by name">
              Name
            </button>
          </th>
          <th>Description</th>
          <th class="col-status">
            <button type="button" class="linklike" data-sort-col="priority" data-sort-type="priority" aria-label="Sort categories by priority">
              Priority
            </button>
          </th>
        ...
        <?php foreach ($categories as $c): ?>
          <tr data-name="<?= h(strtolower($c['name'])) ?>"
            ...
            <td class="col-actions">
              <div class="actions">
                <a class="btn" href="doc_tasks.php?category_id=<?= (int)$c['id'] ?>">Documents</a>
                <a class="btn" href="doc_category_form.php?id=<?= (int)$c['id'] ?>">Edit</a>
                <a class="btn danger"
                   href="doc_category_delete.php?id=<?= (int)$c['id'] ?>"
                   onclick="return confirm('Delete this category and all its documents?');">
                  Delete
                </a>
              </div>
            </td>
...
<div class="card">
  ...
  <div class="table-wrap">
    <table class="table-auto" data-table-key="doc-recent" data-default-sort-col="priority" data-default-sort-dir="desc">
      <thead>
        <tr>
          <th>
            <button type="button" class="linklike" data-sort-col="title" data-sort-type="text" aria-label="Sort documents by title">
              Document
            </button>
          </th>
          <th class="col-project">Category</th>
          <th class="col-status">
            <button type="button" class="linklike" data-sort-col="status" data-sort-type="status" aria-label="Sort documents by status">
              Status
            </button>
          </th>
          <th class="col-status">
            <button type="button" class="linklike" data-sort-col="priority" data-sort-type="priority" aria-label="Sort documents by priority">
              Priority
            </button>
          </th>
        </tr>
        ...
        <?php foreach ($recent_docs as $d): ?>
          ...
          <tr data-title="<?= h(strtolower($d['title'])) ?>"
            ...
            <td class="col-project col-project-wrap">
              ...
              <a class="muted" href="doc_tasks.php?category_id=<?= (int)$d['project_id'] ?>">View category documents</a>
            </td>
