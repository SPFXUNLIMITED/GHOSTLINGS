...
$stmt = $pdo->prepare("SELECT logo_path, logo_thumb FROM vendors WHERE id = ? LIMIT 1");
...
if ($type === 'thumb') {
  ...
  $stored_thumb = (string)($row['logo_thumb'] ?? '');
  $stored_full  = (string)($row['logo_path']  ?? '');
  $src_path = '';
  $detected_mime = '';
  $new_thumb_name = '';
  $thumb_path = '';
  ...
  if ($stored_thumb_exists && $stored_thumb !== $stored_full) {
    ...
    if ($thumb_path !== '' && is_file($thumb_path)) {
      ...
      if ($stored_thumb !== $new_thumb_name) {
        $upd = $pdo->prepare("UPDATE vendors SET logo_thumb = ? WHERE id = ?");
        $upd->execute([$new_thumb_name, $vendor_id]);
      }
      ...
      if ($thumb_ok && is_file($thumb_path)) {
        ...
        error_log("vendor_logo: GD thumbnail generation failed for vendor_id={$vendor_id}, falling back to full image");
        $stored = $stored_full;
      }
    }
  }
} else {
  $stored = (string)($row['logo_path'] ?? '');
...
if (isset($_GET['download'])) {
  ...
  $download_name = 'vendor_' . $vendor_id . '_logo.' . $ext;
