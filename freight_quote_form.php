...
if (empty($_SESSION['freight_quote_form_csrf'])) {
  $_SESSION['freight_quote_form_csrf'] = bin2hex(random_bytes(24));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($_SESSION['freight_quote_form_csrf']) || !hash_equals((string)$_SESSION['freight_quote_form_csrf'], $submitted_csrf)) {
...
<form method="post" class="card" novalidate>
  <input type="hidden" name="csrf_token" value="<?= h($_SESSION['freight_quote_form_csrf']) ?>" />
  <?php if ($is_edit): ?>
  ...
  <div class="card" style="margin-bottom:14px; background:var(--surface-alt, #f8f9fa);">
    ...
    <div class="form-grid length-converter-grid">
      <div>
